    ...
    local function Hop()
        for i=1,100 do
            ...
            for k,v in pairs(huhu) do
                if v.Count <= 4 and k ~= game.JobId and not table.find(CheckJobIdServer(),k) then
...
function TeleportSeabeast(x)
    ...
    if (a-b).Magnitude <= 175 then
        ...
        toTarget(v.HumanoidRootPart.CFrame ,Vector3.new(x.HumanoidRootPart.Position.X,game:GetService("Workspace").Map["WaterBase-Plane"].Position.Y,x.HumanoidRootPart.Position.Z),CFrame.new(x.HumanoidRootPart.Position.X,game:GetService("Workspace").Map["WaterBase-Plane"].Position.Y+200,x.HumanoidRootPart.Position.Z))
    end		
end
...
-- Bypass WalkSpeed Script
WalkSpeedBypass = 300 -- Tốc độ mong muốn (100 là tốc độ cao hơn mặc định)
